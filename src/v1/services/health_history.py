...
from v1.type_defs import ErrorTypeEnum, APIResponse
...
class HealthHistorySummarizationService:
    ...
    def build_summary(self, patient_id: UUID, db: Session) -> APIResponse[HealthHistorySummarySchema]:
      try:  
        ...
        encounter_models = db.query(ClinicalEncounterModel).options(
            selectinload(ClinicalEncounterModel.staff).selectinload(StaffModel.departments)
        ).filter(
