    ...
    for attempt in range(REDIS_CONNECT_RETRIES):
      try:
        ...
        if attempt < REDIS_CONNECT_RETRIES - 1:
          ...
          print(
              "Failed to connect to Redis after multiple attempts. Redis operations will be skipped.")

      except Exception as e:
        print(
            f"An unexpected error occurred during Redis connection attempt {attempt + 1}: {e}")
        self._redis_client = None
